...
```mermaid
flowchart TD
    A[User Opens Dashboard] --> B[Intro Theme Plays Once]
    B --> C[Click 'Start Monitoring']
    C --> D[MediaPipe FaceLandmarker 478 Mesh Initialized]
    D --> E{User State Evaluation}
    
    E -- Staring Blankly / Level Head --> F[Baseline Compliance: 3D Spatial Mosquito Audio]
    E -- Yawn Detected Δlip > 6% --> G[Gangeee Mode: Sepia Shake + Sweat Drop]
    E -- Look-Away / Head Turned > 30% --> H[Eda Mone Mode: Speed Lines + Laser Eyes]
    E -- Head Tilt > 20° --> I[Gravity Tilt Mode: Dizzy Eyes + Tilt Audio]
    E -- Keystroke / Typing Detected --> J[Work Alert: Police Tape + Amba Popup]
    E -- Mouse Movement / Jiggle --> K[Cursor Chaos: Big FaFa Popup + Dizzy Filter]
    E -- Tab Switched / Window Minimized --> L[Tab Treason: Looping Dashamoolam Audio]
    E -- Window Close Attempt --> M[Escape Trap: Fath Ghost Popup + ghost.mp3]
    
    M -- User Cancels / Stays --> N[stay.mp3 Priority Playback]
    L -- User Returns to Tab --> F
```
